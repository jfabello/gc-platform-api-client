...
| deliveryPushed | `boolean` | No | Filter to indicate if push notification is sent |
| socialRatings | `number[]` | No | A set of ratings for Google Business Profile |
| virtualAgentIds | `string[]` | No | A list of virtual agent ids for filtering. |
| empathyScoreCategories | `string[]` | No | A set of Empathy Score Categories for filtering |
| sentimentScoreCategories | `string[]` | No | A set of Sentiment Score Categories  for filtering |
| sentimentTrendCategories | `string[]` | No | A set of Sentiment Trend Categories for filtering |
| contentModerationFlags | `string[]` | No | A set of Content Moderation Flags for filtering |
| sessionExpired | `boolean` | No | Filter to indicate for if session is expired |
| engagementSource | `string[]` | No | The engagement sources used to filter the view |
