# `AgentCopilotAggregationQuery` Definition
...
| filter | [`AgentCopilotAggregateQueryFilter`](agentcopilotaggregatequeryfilter-definition.md) | No | Behaves like a SQL WHERE clause. This is ANDed with the interval parameter. Expresses boolean logical predicates as well as dimensional filters |
...
| views | [`AgentCopilotAggregationView[]`](agentcopilotaggregationview-definition.md) | No | Custom derived metric views |
...
| sortMetric | [`AgentCopilotAggregationSort`](agentcopilotaggregationsort-definition.md) | No | Required when requesting multiple metrics. Only applicable for topN/bottomN query type. |
| limit | `number` | No | How many results you want in an ordered list. Only applicable for topN/bottomN query type. |
