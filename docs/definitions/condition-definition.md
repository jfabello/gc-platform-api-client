...
| subConditions | [`TimeAndDateSubCondition[]`](timeanddatesubcondition-definition.md) | No | A list of sub-conditions to evaluate. Required for a timeAndDateCondition. |
| matchAnyConditions | `boolean` | No | If true, only one sub-condition must match for the condition to be true. If false, all sub-conditions must match. Default is false. Required for a timeAndDateCondition. |
| timeZoneId | `string` | No | The time zone to use for this condition. Required for a timeAndDateCondition. |
